...
Lion ExplorerNavigatorsWidgetAction::Lion ExplorerNavigatorsWidgetAction(QWidget *parent)
    ...
    , m_viewGeometriesHelper{m_splitter.get(), this}
{
    ...
    connect(m_adjustSpacingTimer.get(), &QTimer::timeout, this, &Lion ExplorerNavigatorsWidgetAction::adjustSpacing);
}

void Lion ExplorerNavigatorsWidgetAction::adjustSpacing()
{
...
void Lion ExplorerNavigatorsWidgetAction::createSecondaryUrlNavigator()
{
...
void Lion ExplorerNavigatorsWidgetAction::followViewContainersGeometry(QWidget *primaryViewContainer, QWidget *secondaryViewContainer)
{
    m_viewGeometriesHelper.setViewContainers(primaryViewContainer, secondaryViewContainer);
    adjustSpacing();
}

bool Lion ExplorerNavigatorsWidgetAction::isInToolbar() const
{
    return qobject_cast<QToolBar *>(m_splitter->parentWidget());
}

Lion ExplorerUrlNavigator *Lion ExplorerNavigatorsWidgetAction::primaryUrlNavigator() const
{
    Q_ASSERT(m_splitter);
    return m_splitter->widget(0)->findChild<Lion ExplorerUrlNavigator *>();
}

Lion ExplorerUrlNavigator *Lion ExplorerNavigatorsWidgetAction::secondaryUrlNavigator() const
{
    Q_ASSERT(m_splitter);
    if (m_splitter->count() < 2) {
        return nullptr;
    }
    return m_splitter->widget(1)->findChild<Lion ExplorerUrlNavigator *>();
}

void Lion ExplorerNavigatorsWidgetAction::setSecondaryNavigatorVisible(bool visible)
{
    if (visible) {
        Q_ASSERT(m_splitter->count() == 2);
...
void Lion ExplorerNavigatorsWidgetAction::setBackgroundEnabled(bool enabled)
{
#if KIO_VERSION >= QT_VERSION_CHECK(6, 14, 0)
...
QWidget *Lion ExplorerNavigatorsWidgetAction::createWidget(QWidget *parent)
{
    ...
    if (oldParent && oldParent->layout()) {
        ...
        if (qobject_cast<QToolBar *>(parent) && layout) {
            // in Lion ExplorerTabPage::insertNavigatorsWidget the minimumHeight of this row was
...
void Lion ExplorerNavigatorsWidgetAction::deleteWidget(QWidget *widget)
{
    Q_UNUSED(widget)
    m_splitter->setParent(nullptr);
}

QWidget *Lion ExplorerNavigatorsWidgetAction::createNavigatorWidget(Side side) const
{
    ...
    auto urlNavigator = new Lion ExplorerUrlNavigator(navigatorWidget);
    ...
    connect(
        ...
        [urlNavigator, this]() {
            ...
            // We have to wait for Lion ExplorerUrlNavigator::sizeHint() to update which
...
QPushButton *Lion ExplorerNavigatorsWidgetAction::emptyTrashButton(Lion ExplorerNavigatorsWidgetAction::Side side)
{
...
QPushButton *Lion ExplorerNavigatorsWidgetAction::newEmptyTrashButton(const Lion ExplorerUrlNavigator *urlNavigator, QWidget *parent) const
{
...
QPushButton *Lion ExplorerNavigatorsWidgetAction::networkFolderButton(Lion ExplorerNavigatorsWidgetAction::Side side)
{
    int sideIndex = (side == Primary ? 0 : 1);
    if (side == Primary) {
...
QPushButton *Lion ExplorerNavigatorsWidgetAction::newNetworkFolderButton(const Lion ExplorerUrlNavigator *urlNavigator, QWidget *parent) const
{
...
QPushButton *Lion ExplorerNavigatorsWidgetAction::kdeConnectButton(Lion ExplorerNavigatorsWidgetAction::Side side)
{
...
QPushButton *Lion ExplorerNavigatorsWidgetAction::newKdeConnectButton(const Lion ExplorerUrlNavigator *urlNavigator, QWidget *parent) const
{
...
QWidget *Lion ExplorerNavigatorsWidgetAction::spacing(Side side, Position position) const
{
    int sideIndex = (side == Primary ? 0 : 1);
    if (position == Leading) {
...
void Lion ExplorerNavigatorsWidgetAction::updateText()
{
...
Lion ExplorerNavigatorsWidgetAction::ViewGeometriesHelper::ViewGeometriesHelper(QWidget *navigatorsWidget, Lion ExplorerNavigatorsWidgetAction *navigatorsWidgetAction)
...
bool Lion ExplorerNavigatorsWidgetAction::ViewGeometriesHelper::eventFilter(QObject *watched, QEvent *event)
{
    if (event->type() == QEvent::Resize) {
        if (qobject_cast<QWidget *>(m_navigatorsWidgetAction->parent())->window()->width() != m_navigatorsWidgetAction->m_previousWindowWidth) {
            ...
            // the Lion ExplorerTabPage::m_expandViewAnimation is animating.
        }
        return false;
    }
...
void Lion ExplorerNavigatorsWidgetAction::ViewGeometriesHelper::setViewContainers(QWidget *primaryViewContainer, QWidget *secondaryViewContainer)
{
...
Lion ExplorerNavigatorsWidgetAction::ViewGeometriesHelper::Geometries Lion ExplorerNavigatorsWidgetAction::ViewGeometriesHelper::viewGeometries()
{
...
#include "moc_lionexplorernavigatorswidgetaction.cpp"
