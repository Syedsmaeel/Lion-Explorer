...
#ifndef LIONEXPLORERPART_H
#define LIONEXPLORERPART_H
...
class Lion ExplorerNewFileMenu;
class Lion ExplorerViewActionHandler;
class QActionGroup;
class KFileItemList;
class KFileItem;
class Lion ExplorerPartBrowserExtension;
class Lion ExplorerRemoteEncoding;
class KDirLister;
class Lion ExplorerView;
class Lion ExplorerRemoveAction;

class Lion ExplorerPart : public KParts::ReadOnlyPart
{
    Q_OBJECT
    // Used by konqueror. Technically it means "we want undo enabled if
    // there are things in the undo history and the current part is a lionexplorer part".
...
public:
    explicit Lion ExplorerPart(QWidget *parentWidget, QObject *parent, const KPluginMetaData &metaData, const QVariantList &args);
    ~Lion ExplorerPart() override;

    /**
     * Standard KParts::ReadOnlyPart openUrl method.
     * Called by Konqueror to view a directory in Lion ExplorerPart.
     */
     ...
     * Those names come from the Actions line in lionexplorerpart.desktop,
    ...
    /// Returns the view owned by this part; used by Lion ExplorerPartBrowserExtension
...
private Q_SLOTS:
    ...
    /**
     * Connected to all "Go" menu actions provided by Lion ExplorerPart
     */
...
private:
    ...
    Lion ExplorerPartBrowserExtension *m_extension;
    Lion ExplorerNewFileMenu *m_newFileMenu;
    QAction *m_findFileAction;
    QAction *m_openTerminalAction;
    QString m_nameFilter;
    Lion ExplorerRemoveAction *m_removeAction;
    Q_DISABLE_COPY(Lion ExplorerPart)
};

#endif /* LIONEXPLORERPART_H */
