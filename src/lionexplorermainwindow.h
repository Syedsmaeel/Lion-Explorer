...
#ifndef LIONEXPLORER_MAINWINDOW_H
#define LIONEXPLORER_MAINWINDOW_H
...
class Lion ExplorerBookmarkHandler;
class Lion ExplorerViewActionHandler;
class Lion ExplorerSettingsDialog;
class Lion ExplorerViewContainer;
class Lion ExplorerRemoteEncoding;
class Lion ExplorerTabWidget;
class KFileItem;
class KFileItemList;
class KJob;
class KNewFileMenu;
...
/**
 * @short Main window for Lion Explorer.
 *
 * Handles the menus, toolbars and Lion Explorer views.
 */
class Lion ExplorerMainWindow : public KXmlGuiWindow
{
    Q_OBJECT
    Q_CLASSINFO("D-Bus Interface", "org.kde.lionexplorer.MainWindow")

public:
    Lion ExplorerMainWindow();
    ~Lion ExplorerMainWindow() override;
     ...
     */
    Lion ExplorerViewContainer *activeViewContainer() const;

    /**
     * Returns view container for all tabs
     */
    QVector<Lion ExplorerViewContainer *> viewContainers() const;
     ...
     * Lion Explorer-specific actions, such as "unlock panels".
     */
...
public Q_SLOTS:
    ...
    /**
     * Tries to raise/activate the Lion Explorer window.
     */
     ...
     * Inform all affected lionexplorer components (panels, views) of an URL
     * change.
     */
    ...
    /** Stores all settings and quits Lion Explorer. */
    void quit();

    /**
     * Opens a new tab in the background showing the URL \a url.
     * @return A pointer to the opened Lion ExplorerTabPage.
     */
    Lion ExplorerTabPage *openNewTab(const QUrl &url);
...
private Q_SLOTS:
     ...
     * the given Lion Explorer settings.
     */
    void refreshViews();
    ...
    /** Calls Lion ExplorerViewContainer::setSelectionMode() for m_activeViewContainer. */
    ...
    /** Opens the settings dialog for Lion Explorer. */
    void editSettings();
     ...
     * and necessary actions to be able to use Lion Explorer. This is supposed to hold true even
     ...
     */
    void activeViewChanged(Lion ExplorerViewContainer *viewContainer);
    ...
    /**
     * This slot is called when the user requested to unmount a removable media
     * _not_ from the lionexplorer's places menu (from the notification area for e.g.)
...
private:
    ...
    /**
     * Connects the signals from the created Lion ExplorerView with
     * the Lion ExplorerViewContainer \a container with the corresponding slots of
     * the Lion ExplorerMainWindow. This method must be invoked each
     * time a Lion ExplorerView has been created.
     */
    void connectViewSignals(Lion ExplorerViewContainer *container);
...
private:
     ...
     * assures that all errors are shown in the status bar of Lion Explorer
    ...
    Lion ExplorerTabWidget *m_tabWidget;
    Lion ExplorerViewContainer *m_activeViewContainer;

    Lion ExplorerViewActionHandler *m_actionHandler;
    Lion ExplorerRemoteEncoding *m_remoteEncoding;
    QPointer<Lion ExplorerSettingsDialog> m_settingsDialog;
    Lion ExplorerBookmarkHandler *m_bookmarkHandler;
    ...
    friend class Lion ExplorerMainWindowTest;
};

inline Lion ExplorerViewContainer *Lion ExplorerMainWindow::activeViewContainer() const
{
    return m_activeViewContainer;
}

inline KNewFileMenu *Lion ExplorerMainWindow::newFileMenu() const
{
    return m_newFileMenu;
}

#endif // LIONEXPLORER_MAINWINDOW_H
