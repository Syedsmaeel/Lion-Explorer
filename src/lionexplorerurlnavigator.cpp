...
Lion ExplorerUrlNavigator::Lion ExplorerUrlNavigator(QWidget *parent)
    : Lion ExplorerUrlNavigator(QUrl(), parent)
{
}

Lion ExplorerUrlNavigator::Lion ExplorerUrlNavigator(const QUrl &url, QWidget *parent)
    : KUrlNavigator(Lion ExplorerPlacesModelSingleton::instance().placesModel(), url, parent)
{
    const GeneralSettings *settings = GeneralSettings::self();
    setUrlEditable(settings->editableUrl());
    setShowFullPath(settings->showFullPath());
    setHomeUrl(Lion Explorer::homeUrl());
    setPlacesSelectorVisible(Lion ExplorerUrlNavigatorsController::placesSelectorVisible());
    ...
    setWhatsThis(xi18nc("@info:whatsthis location bar",
                        ...
                        "<link url='help:/lionexplorer/location-bar.html'>click here</link>. "
                        "This will open the dedicated page in the Handbook.</para>"));

    Lion ExplorerUrlNavigatorsController::registerLion ExplorerUrlNavigator(this);

    connect(this, &KUrlNavigator::returnPressed, this, &Lion ExplorerUrlNavigator::slotReturnPressed);
...
Lion ExplorerUrlNavigator::~Lion ExplorerUrlNavigator()
{
    Lion ExplorerUrlNavigatorsController::unregisterLion ExplorerUrlNavigator(this);
}

QSize Lion ExplorerUrlNavigator::sizeHint() const
{
    if (isUrlEditable()) {
...
std::unique_ptr<Lion ExplorerUrlNavigator::VisualState> Lion ExplorerUrlNavigator::visualState() const
{
...
void Lion ExplorerUrlNavigator::setVisualState(const VisualState &visualState)
{
...
void Lion ExplorerUrlNavigator::clearText() const
{
    editor()->lineEdit()->clear();
}

void Lion ExplorerUrlNavigator::setPlaceholderText(const QString &text)
{
    editor()->lineEdit()->setPlaceholderText(text);
}

void Lion ExplorerUrlNavigator::setReadOnlyBadgeVisible(bool visible)
{
...
bool Lion ExplorerUrlNavigator::readOnlyBadgeVisible() const
{
    QWidget *readOnlyBadge = badgeWidget();
    if (readOnlyBadge) {
...
void Lion ExplorerUrlNavigator::slotReturnPressed()
{
...
void Lion ExplorerUrlNavigator::keyPressEvent(QKeyEvent *keyEvent)
{
...
#include "moc_lionexplorerurlnavigator.cpp"
