...
#ifndef LIONEXPLORER_TAB_PAGE_H
#define LIONEXPLORER_TAB_PAGE_H
...
class Lion ExplorerNavigatorsWidgetAction;
class Lion ExplorerViewContainer;
class QVariantAnimation;
class KFileItemList;
class Lion ExplorerTabPageSplitter;

class Lion ExplorerTabPage : public QWidget
{
    Q_OBJECT

public:
    explicit Lion ExplorerTabPage(const QUrl &primaryUrl, const QUrl &secondaryUrl = QUrl(), QWidget *parent = nullptr);
     ...
     */
    Lion ExplorerViewContainer *primaryViewContainer() const;

    /**
     * @return The secondary view container, can be 0 if split view is disabled.
     */
    Lion ExplorerViewContainer *secondaryViewContainer() const;

    /**
     * @return Lion ExplorerViewContainer of the active view
     */
    Lion ExplorerViewContainer *activeViewContainer() const;

    /**
     * @return Lion ExplorerViewContainer of the inactive view
     * if split view is enabled, or nullptr otherwise.
     */
    Lion ExplorerViewContainer *inactiveViewContainer() const;
    ...
    /**
     * Connects a navigatorsWidget to this. It will be connected to the Lion ExplorerViewContainers
     ...
     */
    void connectNavigators(Lion ExplorerNavigatorsWidgetAction *navigatorsWidget);

    /**
     * Makes it so this tab and its Lion ExplorerViewContainers aren't controlled by any
     * UrlNavigators anymore.
     */
    void disconnectNavigators();

    void insertNavigatorsWidget(Lion ExplorerNavigatorsWidgetAction *navigatorsWidget);

    /**
     * Marks the items indicated by \p urls to get selected after the
     * directory Lion ExplorerView::url() has been loaded. Note that nothing
     * gets selected if no loading of a directory has been triggered
     * by Lion ExplorerView::setUrl() or Lion ExplorerView::reload().
     */
    void markUrlsAsSelected(const QList<QUrl> &urls);

    /**
     * Marks the item indicated by \p url to be scrolled to and as the
     * current item after directory Lion ExplorerView::url() has been loaded.
     */
...
private:
     ...
     */
    Lion ExplorerViewContainer *createViewContainer(const QUrl &url) const;
     ...
     */
    void startExpandViewAnimation(Lion ExplorerViewContainer *expandingContainer);

    /**
     * Connect/disconnect Lion ExplorerView::activated → slotViewActivated for all
...
private:
    Lion ExplorerTabPageSplitter *m_splitter;

    QPointer<Lion ExplorerNavigatorsWidgetAction> m_navigatorsWidget;
    QPointer<Lion ExplorerViewContainer> m_primaryViewContainer;
    QPointer<Lion ExplorerViewContainer> m_secondaryViewContainer;

    Lion ExplorerViewContainer *m_expandingContainer;
...
class Lion ExplorerTabPageSplitterHandle : public QSplitterHandle
{
    Q_OBJECT

public:
    explicit Lion ExplorerTabPageSplitterHandle(Qt::Orientation orientation, QSplitter *parent);
...
class Lion ExplorerTabPageSplitter : public QSplitter
{
    Q_OBJECT

public:
    explicit Lion ExplorerTabPageSplitter(Qt::Orientation orientation, QWidget *parent);

protected:
    QSplitterHandle *createHandle() override;
};

#endif // LIONEXPLORER_TAB_PAGE_H
