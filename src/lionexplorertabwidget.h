...
#ifndef LIONEXPLORER_TAB_WIDGET_H
#define LIONEXPLORER_TAB_WIDGET_H
...
class Lion ExplorerTabWidget : public QTabWidget
{
    Q_OBJECT
...
public:
     ...
     */
    explicit Lion ExplorerTabWidget(Lion ExplorerNavigatorsWidgetAction *navigatorsWidget, QWidget *parent);
     ...
     */
    Lion ExplorerTabPage *currentTabPage() const;
     ...
     */
    Lion ExplorerTabPage *nextTabPage() const;
     ...
     */
    Lion ExplorerTabPage *prevTabPage() const;

    /**
     * @return Tab page at the given \a index (can be 0 if the index is out-of-range)
     */
    Lion ExplorerTabPage *tabPageAt(const int index) const;
...
public Q_SLOTS:
     ...
     */
    Lion ExplorerTabPage *openNewTab(const QUrl &primaryUrl,
                               const QUrl &secondaryUrl = QUrl(),
                               Lion ExplorerTabWidget::NewTabPosition position = Lion ExplorerTabWidget::NewTabPosition::FollowSetting);
...
private Q_SLOTS:
    /**
     * Opens the tab with the index \a index in a new Lion Explorer instance and closes
     * this tab.
     */
    ...
    /**
     * Calls Lion ExplorerTabPage::setCustomLabel(label) for the tab at @p index
     * and propagates that change to the Lion ExplorerTabBar.
     * @see Lion ExplorerTabPage::setCustomLabel().
     */
...
private:
     ...
     */
    QString tabName(Lion ExplorerTabPage *tabPage) const;
     ...
     */
    Lion ExplorerViewContainer *viewContainerAt(ViewIndex viewIndex) const;
     ...
     */
    Lion ExplorerViewContainer *activateViewContainerAt(ViewIndex viewIndex);
...
private:
    QPointer<Lion ExplorerTabPage> m_lastViewedTab;
